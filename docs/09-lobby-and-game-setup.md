...
2. **Secrets**: each human seat (and each bot host, for each of its bots) generates `masterSecret`s. For each one it computes and publishes a `SeatCommitments` package:
   ...
   - `escrowShareHashes[]`.
3. **Escrow**: send Shamir shares via `PRIVATE`. Recipients reply with signed `SHARE_ACK { dealerSeat, H(share) }`.
...
5. **Genesis draft**: every peer builds the identical `Genesis` object:
   `{ protocolVersion, engineVersion, config, seats(with pubkeys, names, colours, kinds, botHosts), genesisSeed, commitments, shareAcks, createdAt(host clock, informational) }`.
   Each human signs `H(genesis)` and broadcasts the signature.
...
- A human seat's key is the peer identity key.
