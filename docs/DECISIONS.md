...
## 2026-09-25: Verify hidden information per move, not at the end of the game

- Stage: 07 design, before implementation.
- Request: Detect cheating on the move where it happens instead of waiting for the end-of-game audit.
- Decision: Hold every seat's hidden hand as public Pedersen commitments to per-type counts. Spends the bounds can't guarantee carry range proofs, count reveals carry Schnorr openings, and steals carry one-hot and beacon-index OR proofs with the opening sealed to the thief inside the log. Deck shuffles carry a 64-round Fiat–Shamir cut-and-choose single-key shuffle proof plus per-position locking DLEQs. Escrow uses Shamir over the scalar field with Feldman commitments, so shares are checked in the genesis ceremony. The end-of-game reveal remains for the omniscient replay and as a defence-in-depth re-check.
- Alternatives: Keep hash-commitment chains and detect lies at the end-of-game audit; use a Bayer–Groth shuffle argument or Bulletproofs now.
- Reason: Detection at the end lets a cheater influence the whole game and wastes everyone's time. With these proofs, an invalid input is never applied. The only check left for later is whether an escrowed secret matches the keys a seat used, because HKDF derivations can't be proven cheaply; it matters only once that seat has left. Cut-and-choose and bit-decomposition proofs are simple enough at Catan's sizes (≤ 34-card decks, counts < 64); Bayer–Groth and Bulletproofs stay available if performance targets are missed.

## 2026-09-25: Strict agreement and recoverable voting state

- Stage: 06 design and foundations, with dependent requirements corrected in stages 07, 09 and 10.
- User decision: Pause when safe agreement is impossible. The question disclosed that three-human games may need to pause and two-human games cannot continue alone.
- Finding: A three-seat majority can certify conflicting histories with one equivocating voter. Signed acknowledgements alone also do not preserve hidden commits across proposer changes or erased vote records. The original two-player takeover promise contradicted its old-quorum requirement.
- Decision: Use proposal, prevote and precommit rounds with persistent locks, following the locking rules in [Tendermint Algorithm 1](https://arxiv.org/pdf/1807.04938). For one through six human voters, the quorum is 1, 2, 3, 3, 4, 4. Ordering tolerates one Byzantine voter; two-/three-human games sacrifice availability after a required voter leaves. Bots never vote. Hidden-information privacy has its own threshold.
- Decision: Hash a nonrecursive genesis body including a fresh ceremony nonce and explicit stub/verified mode. Bind all commands to its full digest and exact parent. Hash agreed entry values independently of proposer/round signatures. Publish engine effects only after certified commitment. Snapshots require full replay of protocol metadata before voting resumes.
- Decision: Persist proposals, votes, highest entered rounds, locks and certificates before dependent transmission. Use a per-game voting key independent of the escrowed master secret. Store loss cannot be repaired by guessing the highest outstanding vote from other peers. An imported old save cannot reactivate a key; active-device migration uses a certified replacement to a fresh destination key.
- Decision: Membership stays fixed within a height. The old set certifies a transition, which becomes active at the next height. An integer epoch plus the certified parent/value binding avoids a recursive readiness hash. Joining keys sign readiness over the parent, next epoch and proposed member list. Recovery authorization removes and freezes the departed seat before shares are released; a later certified result activates its bot or records failed recovery.
- Decision: Do not distribute escrow in games starting with two or three humans. Their strict quorum cannot authorize an absent-seat takeover, and two-player escrow would reveal the hand to its opponent immediately. Larger games retain sealed shares in genesis for later recovery. Document permanent privacy loss and the ability of a share holder to stall recovery.
- Review: Read-only Claude reviews are saved beside their prompts in `verification/stage06/`. Adopted findings include monotonic persisted rounds, lost-store refusal, halting on unrepaired certified-state disagreement, per-height membership activation and separate recovery authorization. The proposed crash-only majority fallback was declined under the user's choice. The suggestion to discard proposer envelopes was unnecessary for agreement because value hashes already exclude them; committed-log comparisons use values, and certificates may differ. Local timing checks gate fresh votes, while objective validation governs justified proposals and certificates.
- Evidence so far: Strict signatures, canonical schemas, genesis/log rejection paths, vote/certificate validation, wire bounds and deterministic network tests pass. No claim of completed consensus or P2P acceptance is made at this checkpoint.

## 2026-09-25: Network simulation counts

- Request: Avoid large random test counts that add runtime without enough additional coverage.
- Decision: Stage 06 initial acceptance requires 20 complete games in each of the nine scenarios, 180 games total, with zero divergence. Retain the targeted tests for equivocation, hidden certificates, persistent locks, storage failure, repair and small-population quorum loss. A random game count does not prove agreement.
- Decision: Run five games per scenario on pushes and pull requests, and 20 nightly with rotating deterministic game indices. Keep larger manually dispatched runs available. Preserve seed ranges and source fingerprints in reports so failures can be reproduced.
- Reason: Initial four-peer full games took roughly 32–41 seconds locally. The original 1,000-game acceptance and 200/5,000-game CI counts would repeat similar paths at substantial cost. The smaller batches cover complete games across varied boards and decisions while the focused tests exercise the specific protocol failure modes.

## 2026-09-25: Protocol implementation review follow-up

- Stage: 06. The approved read-only Claude review and its follow-up are recorded in `verification/stage06/`.
- Decision: Persist accepted objective accusations and the first proven offender together with voting state. Carry the proof through a competing gameplay commit. Verify older evidence against its certified historical parent before proposing exclusion. A verified conflicting history or second distinct proven offender causes a durable terminal halt.
- Decision: Check local signatures embedded in certificates, proposal justifications, entries and accusation evidence against the retained signing records. A conflicting or unknown current-height signature stops signing. Retain at most two conflicting proposals per round, while preserving the separate proofs needed for locks and justified values.
- Decision: Congestion drops excess packets without misconduct strikes. Malformed messages still count against their sender. Bound expensive replay requests per peer and require actual certified progress before following continued sync responses. Bound pending commands per author so one sender cannot evict every other author.
- Boundary: Stage 06 accepts an injected fresh game key and journal; it cannot prevent a caller from copying that raw key into a different empty store. Production key ownership remains a Stage 10 requirement. A damaged derived context that prevents stored-proposal validation stops the controller and preserves its journal for fresh-session replay. Simulation evidence does not prove hidden facts or deadlines; real games require Stage 07 proofs before commitment.
- Reason: A sound voting rule also needs durable fault decisions and bounded handling of hostile input. These changes preserve the strict quorum and committed prefix. They do not treat a local timeout, transport failure or unverified snapshot as authority to change history.

## 2026-09-25: Repeated evidence and interrupted submissions

- Stage: 06 implementation review follow-up.
- Decision: Evidence against an already excluded proposer is idempotent. Restore clears a matching stale pending accusation through a persisted transition after verifying its retained proof. A first proof received inside a proposal persists across a competing gameplay commit. A second distinct offender halts before further votes or commit effects, including when recovering an older journal that missed the immediate halt.
- Decision: A transport failure or disposal after accepting a signed submission reports an unknown outcome. It is not cancellation. Historical proposals and old certificates authenticate before consuming bounded replay-work slots; restart retransmissions send newer rounds first.
- Simulation: The crash scenario waits until surviving voters know an applicable input before testing proposer replacement. The malicious-proposer scenario leaves the normal attacker's session halted and uses a separate command-only client for subsequent game choices. Assertions compare the three honest histories and require the exact faulty client's signed commands to appear in the certified log.
- Verification: The confirmed review traces have regressions. Full local checks pass 534 tests. The final acceptance batch runs once in GitHub Actions with twenty games per scenario, avoiding a duplicate long local batch. Acceptance remains pending until that run passes.
