# Settlement and city placement UI

The user now wants the same explicit on-board preview/confirmation for settlements and cities, and says city building indicators are hard to see. Author a small concrete Pixi design/code proposal for vertex targets and settlement/city previews. Read-only tools only, do not edit files. Keep the response below 1,500 words, focused on drawing and sizing, with a short rationale.

Inspect current `packages/renderer/src/BoardRenderer.ts`, `types.ts`, `apps/web/src/features/game/GameActions.tsx`, and the style guide below. You previously proposed the approved road affordance: an inset dark neutral lane with three white dashes, edge layers below buildings, and a thin dark/white outline around a candidate road. Those are now implemented and visually approved. Keep roads, ports, sea, tokens, and larger dice unchanged.

Current buildings have player-colored shape markers and white original SVG building artwork. At hexSize54, settlement artwork is24px and its marker nominal radius17px; city artwork is28px and marker radius19px. Shapes include circle, square, triangle, and diamond. Existing vertex highlights are translucent mint circles about13.5px radius with a3px stroke, drawn above buildings. City upgrade targets blend into the existing settlement and terrain.

Need two distinct states:

- Available settlement sites must be obvious yet restrained, without filling the whole corner or obscuring nearby ports.
- Available city upgrades must visibly mark the eligible existing settlement outside its silhouette. Consider a neutral dark/white broken ring or corner brackets and a compact upgrade cue. Use your judgment, avoid bulky slabs, glow, or decorations. It must work over every terrain, on light/dark themes and on phone-sized boards.
- A selected candidate shows an honest preview of the actual settlement or city at its intended position, plus a selection outline. City preview must make the upgrade visible before Confirm. Existing legal IDs, engine validation, privacy, and revision guards stay unchanged. The app/renderer teams are generalizing the preview API; you can propose a small piece-kind field, not engine rule logic.
- World-coordinate graphics scale with the board. Controls are an already implemented compact DOM popover beside the selected site, clamped to board intersected with the viewport and kept in sync after pan/zoom/resize. Keep that popover and its single Confirm/Cancel pair. Setup settlements also require confirmation. Do not propose a replacement dock or broader React rewrite.
- No unintended commit on first selection. Redraw only changed layers, avoid empty Graphics, respect reduced motion, and ensure the existing piece is not drawn over its upgrade preview.

Give exact dimensions/colors/alpha and concise code for available vertex markers plus candidate preview. Original graphics only, no new dependencies. The following style guide supplies the overall visual constraints.
