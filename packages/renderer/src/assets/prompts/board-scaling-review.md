Review the current Hexfield Pixi board's visual scaling and give a concrete correction plan. READ ONLY. Do not edit any file or author game rules.

The user wants clean, readable, modern board graphics. You generated original port marker.svg and jetty.svg. Ports must clearly connect the two eligible coast vertices to an offshore badge. Recent integration revealed wrong inward-facing arms, oversized icons covering ratios, and now general scaling mismatches between labels and artwork. The first two issues were just corrected, but the scaling model still needs review. Renderer files are frozen while you review.

Read these files:

- packages/renderer/src/BoardRenderer.ts
- packages/renderer/src/harborLayout.ts and harborLayout.test.ts
- packages/renderer/src/tokenLayout.ts
- packages/renderer/src/assets/terrainTextures.ts
- packages/renderer/src/assets/harbors/{marker,jetty}.svg
- packages/renderer/src/types.ts
- apps/web/src/style.css for canvas container dimensions

Inspect the user's screenshots with Read:

- /var/folders/gd/b0yk1bp93z398wwsgy13q29w0000gn/T/codex-clipboard-dm6kiB.png (zoomed-out board: oversized port badges and numbers)
- /var/folders/gd/b0yk1bp93z398wwsgy13q29w0000gn/T/codex-clipboard-CqXW28.png (zoomed-in coast: badge tiny relative to arms and tile)
- /var/folders/gd/b0yk1bp93z398wwsgy13q29w0000gn/T/codex-clipboard-xtX41z.png (better badge proportion at one zoom, but numbers differ)

Geometry contract: jetty viewBox 0 0 128 112 has shoreline anchors A=(16,104), B=(112,104), and offshore hub H=(64,28). Its arms stop about (51,48)/(77,48), leaving a radius22 gap for a badge. Marker viewBox0 0 64 64 is upright at H, with face x11..53,y9..47. Resource icon and ratio are renderer children. Any mapping must keep A/B exactly on the engine's coastal vertices. H must be seaward; all6edge orientations and reversed endpoint order must work.

Latest fix uses signed jetty Y reflection, and badge/icon/ratio are now one inversely-camera-scaled Container with marker40px/icon13px/ratio12px. Token numbers/pips are also inversely scaled, while token faces, pieces and jetty are board-scaled. This is the suspected source of the new mismatch.

We need one coherent policy across full zoom range, desktop and390x844phone. Ports, arms, resource symbols/ratios, token faces/numbers/pips must remain proportional and visibly attached. All coastline ports must be visible at initial fit. Accessibility target44CSSpx remains independent from artwork, and phone users can zoom; do not enforce constant screen font size in a way that breaks art proportions. Normal default fit should be as readable as practical; arbitrary zoom-out can naturally reduce board text. Avoid elaborate zoom-dependent geometry or multiple special cases. Prefer board-space groups with consistent base sizes; upright port labels may need composition independent of arm rotation but share camera scale. Preserve dirty layer redraw policy and cached textures.

Return:

1. Specific defects with file:line references, including any remaining arm/badge attachment issue.
2. One recommended sizing policy, concrete world-space dimensions and container organization, token font/pip proportions, ratio/icon offsets, camera fit implications.
3. A short visual verification matrix across allcoast directions, zoommin/fit/max, desktop andphone. Explain any readability tradeoff honestly.
