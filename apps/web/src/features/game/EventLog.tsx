...
export function EventLog({
  ...
  const eventText = (event: GameEvent): string => {
    const actor = 'seat' in event && typeof event.seat === 'number' ? playerName(event.seat) : null;
    if (event.type === 'roadBuilt' && actor) return t('log:roadBuilt', { player: actor });
    if (event.type === 'settlementBuilt' && actor)
      return t('log:settlementBuilt', { player: actor });
    ...
    if (event.type === 'resourcesProduced' && record(event.bySeat)) {
      const gains = Object.entries(event.bySeat).flatMap(([seat, counts]) => {
        ...
        return cards.length ? [`${playerName(Number(seat))}: ${cards.join(', ')}`] : [];
      });
    ...
    if (
      ...
      return t('log:resourceStolen', {
        thief: playerName(event.thief),
        victim: playerName(event.victim),
      });
    if (event.type === 'turnStarted' && actor) return t('log:turnStarted', { player: actor });
    if (event.type === 'tradeOffered' || event.type === 'tradeProposed')
      return t('log:tradeOffered');
    ...
    if (event.type === 'gameEnded' && typeof event.winner === 'number') {
      return t('log:gameEnded', { player: playerName(event.winner) });
    }
